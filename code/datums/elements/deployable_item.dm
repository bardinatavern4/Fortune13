...
/datum/element/deployable_item/Attach(datum/target, _deploy_type, _deploy_time)
	...
	RegisterSignal(attached_item, COMSIG_ITEM_ATTACK_SELF, .proc/deploy)
...
/datum/element/deployable_item/proc/finish_deploy(datum/source, mob/user, location, direction)
	...
	if(user)
		...
		if(!do_after(user, deploy_time, TRUE, attached_item))
			return
	...
	ENABLE_BITFIELD(attached_item.item_flags, IS_DEPLOYED)
...
/datum/element/deployable_item/proc/finish_undeploy(datum/source, mob/user)
	...
	if(!do_after(user, deploy_time, TRUE, deployed_machine))
		return
	user.unset_interaction()
	user.put_in_hands(attached_item)

	DISABLE_BITFIELD(attached_item.item_flags, IS_DEPLOYED)
	UnregisterSignal(deployed_machine, COMSIG_ITEM_ATTACK_SELF)
