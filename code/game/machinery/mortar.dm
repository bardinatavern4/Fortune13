//ported from TGMC, code is too old to easily find the original coder unfortunately.
...
/obj/machinery/deployable/mortar/attack_hand(mob/living/user)
	...
	if(busy)
		...
		return
	if(firing)
		to_chat(user, "<span class='warning'>[src]'s barrel is red hot, if you fire it again so soon it might blow up!.</span>")
		return
...
/obj/machinery/deployable/mortar/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/mortal_shell))
		...
		if(istype(A) && isclosedturf(A))
			to_chat(user, "<span class='warning'>You can't hit targets that are indoors!.</span>")
			return

		user.visible_message("<span class='notice'>[user] prepares to fire \a [mortar_shell.name]! Cover your ears!</span>",
...
/obj/item/mortar_kit
	name = "\improper M29 mortar"
	desc = "A crew served mortar, used by the pre-war United States. These scavenged mortars are often called 'Suicide Pills' by members of the NCR Army, due to their tendency to detonate and kill the crew."
	icon = 'icons/heavyweapons/mortar.dmi'
	icon_state = "mortar"
...
/obj/item/mortar_kit/attack_self(mob/user)
	var/area/B = get_area(src)
	if(istype(B) && isclosedturf(B))
...
/obj/item/mortal_shell/warcrimes
	name = "\improper 80mm mustard gas mortar shell"
	desc = "An 80mm mortar shell, using a deadly mustard gas payload. Only authorized for use in extreme emergencies. Manufactured at Vault City"
	icon_state = "mortar_ammo_gas"
	var/datum/effect_system/smoke_spread/mustard/smoke

/obj/item/mortal_shell/warcrimes/Initialize()
	. = ..()
	src.smoke = new /datum/effect_system/smoke_spread/mustard
	src.smoke.attach(src)
	
/obj/item/mortal_shell/warcrimes/detonate(turf/T)
	explosion(T, 0, 0, 1, 3)
...
/obj/structure/closet/crate/mortar_ammo
	name = "\improper M29 mortar ammo crate"
	desc = "A salvaged crate containing rare pre-war mortar ammo, and low pressure mustard gas shells manufactured at Vault City."

/obj/structure/closet/crate/mortar_ammo/full/PopulateContents()
	new /obj/item/mortal_shell/he(src)
	new /obj/item/mortal_shell/he(src)
	new /obj/item/mortal_shell/warcrimes(src)
	new /obj/item/mortal_shell/warcrimes(src)
	new /obj/item/mortal_shell/warcrimes(src)
	new /obj/item/mortal_shell/warcrimes(src)
	new /obj/item/mortal_shell/warcrimes(src)
...
/obj/structure/closet/crate/mortar_ammo/mortar_kit
	name = "\improper M29 mortar crate"
	desc = "A crate containing a mortar and accompanying shells. This one is issued to 'Stalker' Battalion and has been issued with chemical shells."
